...
export default function MortgageChat({
      ...
      if (error.status !== 0 && error.code !== "TIMEOUT") {
        setMessages((prev) => [
          ...prev,
          {
            role: "assistant",
            content:
              `I hit an error: ${error.message}. Please try again in a moment.`,
          },
        ]);
      }
    },
      ...
      <div className="flex h-full w-full flex-col bg-white">
        ...
        <div className="flex-1 px-4 py-4">
          ...
          <div className="space-y-3">
            ...
            >
              <span className="text-sm font-bold leading-[21px] text-white">Fill in Application</span>
            </button>
  ...
  return (
    <div className="flex h-full w-full flex-col bg-white">
      {/* Header */}
      <div className="flex items-center justify-between bg-white px-4 py-4 border-b border-gray-100">
        <button onClick={() => setChatStarted(false)} className="flex h-12 w-12 items-center justify-center">
          <svg width="18" height="16" viewBox="0 0 18 16" fill="none" className="h-6 w-6">
            <path fillRule="evenodd" clipRule="evenodd" d="M18 8C18 8.41421 17.6642 8.75 17.25 8.75H2.56031L8.03063 14.2194C8.32368 14.5124 8.32368 14.9876 8.03063 15.2806C7.73757 15.5737 7.26243 15.5737 6.96937 15.2806L0.219375 8.53063C0.0785422 8.38995 -0.000590086 8.19906 -0.000590086 8C-0.000590086 7.80094 0.0785422 7.61005 0.219375 7.46937L6.96937 0.719375C7.26243 0.426319 7.73757 0.426319 8.03063 0.719375C8.32368 1.01243 8.32368 1.48757 8.03063 1.78062L2.56031 7.25H17.25C17.6642 7.25 18 7.58579 18 8Z" fill="#1C140D"/>
          </svg>
        </button>
        <h1 className="flex-1 pr-12 text-center text-lg font-bold leading-6 text-mortgage-dark">Mortgage Chat</h1>
        <span className="text-xs text-mortgage-brown">Session: {sessionId.slice(-8)}</span>
      </div>

      {/* Summary */}
      {(mortgageResults || chatStarted) && (
        <div className="border-b border-gray-100 px-4 py-4">
          <h2 className="mb-6 text-lg font-bold text-mortgage-dark">Mortgage Calculation Summary</h2>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4">
        ...
        {showWelcome && (
          <div className="mb-6 max-w-[280px] mr-12">
            <div className="mb-1">
              <span className="text-sm text-left text-mortgage-brown">Mortgage Bot</span>
            </div>
            <div className="rounded-xl p-3 bg-mortgage-cream text-mortgage-dark">
              <p className="text-base leading-6">
                👋 Hi! I’m your mortgage assistant. Ask me anything about rates, terms, or the application.
              </p>
            </div>
          </div>
        )}
            ...
            return (
              <div key={idx} className={`flex ${isUser ? "justify-end" : "justify-start"}`}>
                <div className={`max-w-[280px] ${isUser ? "ml-12" : "mr-12"}`}>
                  <div className="mb-1">
                    <span className={`text-sm ${isUser ? "text-right" : "text-left"} text-mortgage-brown`}>
                      {isUser ? "You" : "Mortgage Bot"}
                    </span>
                  </div>
                  <div className={`rounded-xl p-3 ${isUser ? "bg-primary text-white" : "bg-mortgage-cream text-mortgage-dark"}`}>
                    <p className="text-base leading-6 whitespace-pre-wrap break-words">{m.content}</p>
                  </div>
                </div>
              </div>
            );
          ...
          {isLoading && (
            <div className="flex justify-start">
              <div className="max-w-[280px] mr-12 rounded-xl p-3 bg-mortgage-cream text-mortgage-dark border border-gray-200">
                ...
                <div className="text-xs text-mortgage-brown mt-1">This may take up to 2 minutes.</div>
              </div>
            </div>
          )}
          ...
          {hasError && (
            <div className="flex justify-center">
              <div className="max-w-md px-4 py-3 bg-red-50 border border-red-200 rounded-lg text-center">
                <p className="text-red-800 text-sm mb-2">Something went wrong sending the last message.</p>
                {canRetry && (
                  <button
                    onClick={handleRetry}
      ...
      <div className="border-t border-gray-100 p-4">
        <div className="flex items-center gap-2 mb-4">
          ...
          >
            <span className="text-sm font-bold leading-[21px] text-white">{isLoading ? "…" : "Send"}</span>
          </button>
        </div>
        ...
        {messages.length >= 1 && (
          <div className="flex gap-3">
            ...
            >
              <span className="text-sm font-bold leading-[21px] text-white">Continue</span>
            </button>
            <button
              onClick={() => setChatStarted(false)}
              className="flex h-10 min-w-[84px] items-center justify-center rounded-2xl bg-mortgage-cream px-4"
            >
              <span className="text-sm font-bold leading-[21px] text-mortgage-dark">Cancel</span>
            </button>
          </div>
        )}
      </div>
