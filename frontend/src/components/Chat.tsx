    ...
    <div className={`flex flex-col h-full bg-white border border-gray-200 rounded-lg shadow-sm ${className}`}>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        {state.messages.length === 0 && (
          <div className="text-center text-gray-500 py-8">
            <p>Start a conversation with the MedFi assistant.</p>
            <p className="text-sm mt-2">Your session ID is preserved for context across messages.</p>
          </div>
        )}
      ...
      <form onSubmit={handleSubmit} className="border-t border-gray-200 p-4">
        ...
        <div className="mt-2 text-xs text-gray-500">
          ...
          {state.messages.length > 0 && (
            <span className="ml-4">• {state.messages.length} messages in session</span>
          )}
        </div>
      </form>
    </div>
